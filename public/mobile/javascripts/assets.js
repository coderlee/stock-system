	...
	methods: {
		...
		listAjax(texts) {
			...
			initDataTokens({
				...
				// let change_wallet=res.message.change_wallet.balance;
				// let legal_wallet=res.message.legal_wallet.balance;
				// let totalCount=0;
				// for(let i=0;i<lever_wallet.length;i++){
				//     if(lever_wallet[i].currency==1 || lever_wallet[i].currency==2 || lever_wallet[i].currency==3 ){
				//     totalCount+=lever_wallet[i].lever_balance*1+lever_wallet[i].lock_lever_balance*1;
				//     }
				// }
				// for(let i=0;i<micro_wallet.length;i++){
				//     if(micro_wallet[i].currency==1 || micro_wallet[i].currency==2 || micro_wallet[i].currency==3 ){
				//     totalCount+=micro_wallet[i].micro_balance*1+micro_wallet[i].lock_micro_balance*1;
				//     }
				    
				// }
				// for(let i=0;i<change_wallet.length;i++){
				//     if(change_wallet[i].currency==1 || change_wallet[i].currency==2 || change_wallet[i].currency==3 ){
    // 				    totalCount+=change_wallet[i].change_balance*1+change_wallet[i].lock_change_balance*1;
				//     }
				// }
				// for(let i=0;i<legal_wallet.length;i++){
				//     if(legal_wallet[i].currency==1 || legal_wallet[i].currency==2 || legal_wallet[i].currency==3 ){
    // 				    totalCount+=legal_wallet[i].legal_balance*1+legal_wallet[i].lock_legal_balance*1;
				//     }
