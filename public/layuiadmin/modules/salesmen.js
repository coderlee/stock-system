...
layui.define(['table', 'form'], function(exports){
    ...
    table.render({
        ...
        ,cols: [[
            ...
            ,{title: '操作', width: 600, align:'center', fixed: 'right', toolbar: '#table-useradmin-webuser'}
        ]]
        ,page: true
        ,limit: 30
        ,height: 'full-320'
    ...
    table.on('tool(LAY-user-manage)', function(obj){
        ...
        }else if (obj.event == 'this-risk'){
            console.log(obj)
            var agent_id = obj.data.id;
            var risk=obj.data.risk==1?0:1;
            admin.req( {
                type : "POST",
                url : '/agent/opencloseRisk',
                dataType : "json",
                data : {agent_id:agent_id,risk:risk},
                done : function(result) { //返回数据根据结果进行相应的处理
                    layer.msg(result.msg, {icon: 6 });
                    //提交 Ajax 成功后，关闭当前弹层并重载表格
                    layui.table.reload('LAY-user-manage'); //重载表格
        ...
        }else if (obj.event == 'this-del') {
            layer.confirm('确定删除代理商吗？', {
              btn: ['确定', '取消'] //可以无限个按钮
