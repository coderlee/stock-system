...
//<--LDH-->
    ...
    Route::get('api/bank/interest','Api\BankController@dispatchInterest');//派息接口
...
//Route::any('api/currency/new_timeshar', 'Api\CurrencyController@newTimeshars')->middleware(['cross']); //K线分时数据，对接tradeingview
    ...
    Route::get('api/user/numberPromoters', 'Api\UserController@numberPromoters');//导入会员
...
//******************************api接口需要登录的**********************
    Route::group(['prefix' => 'api', 'middleware' => ['check_api', /*'check_user'*/]], function () {
        ...
        //跟单
        Route::get('/transaction/gd_user_list', 'Api\TransactionController@GdUserList');
        Route::post('/transaction/gd_order', 'Api\TransactionController@newGdOrder');
        Route::post('/transaction/gd_order/cancel', 'Api\TransactionController@cancelGdOrder');


        Route::post('/bank/account','Api\BankController@newAccount');//创建账户
        Route::get('/bank/my_account','Api\BankController@myAccount');//获取我的账号
        Route::post('/bank/transaction','Api\BankController@saveMoney');//转账进账户
        Route::get('/bank/bank_log','Api\BankController@bankLog');//获取我的账户明细
        Route::get('/bank/week_profit','Api\BankController@weekProfit');//获取我的账户明细
        Route::get('/bank/test','Api\BankController@test');//测试路由
        Route::post('/bank/deposit','Api\BankController@timeDeposit');//存单
        Route::get('/bank/deposit_order','Api\BankController@myDepositOrder');//获取我的存单
        Route::post('/bank/deposit/cancel','Api\BankController@cancelOrder');//毁约存单
        Route::post('/bank/withdraw','Api\BankController@withdraw');//提现
...
//     Route::get('/bank','Api\UserController@bankList');//获取收款方式
        ...
        Route::post('editLegalPrice', 'Api\LegalDealController@editLegalPrice'); 
        ...
        Route::get('user/sign', 'Api\UserController@daySign');//签到
        ...
        Route::get('/wallet/sign', 'Api\WalletController@signday');//签到
...
//管理后台
    Route::group(['prefix' => 'admin', 'middleware' => ['admin_auth']], function () {
          ...
          //跟单
        Route::get('/users/gd_user/view','Admin\UserController@gdUserListView');//页面
        Route::get('/users/gd_user/edit','Admin\UserController@gdUserEditView');//页面
        Route::get('/users/gd_user/list','Admin\UserController@gdUserList');//页面数据
        Route::post('/users/gd_user/add','Admin\UserController@postGdUser');//新增/编辑跟单用户
        ...
        Route::get('/user/user_add', 'Admin\UserController@addshow');
