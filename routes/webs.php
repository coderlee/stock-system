...
Route::get('api/env.json', function () {
    $request = request();
    $result = \App\Utils\RPC::apihttp($request->server('HTTP_HOST') . '/env.json');
...
Route::group(['prefix' => 'api', 'middleware' => ['check_api']], function () {
    ...
    Route::post('wallet/out', 'Api\WalletController@postWalletOut')->middleware(['demo_limit', 'lever_hold_check']);//提交提币信息
...
//     Route::get('/bank','Api\UserController@bankList');//获取收款方式
    ...
    Route::post('/transaction/in', 'Api\TransactionController@in');//买入
    Route::post('/transaction/out', 'Api\TransactionController@out');//卖出

    Route::post('/lever/deal', 'Api\LeverController@deal'); //合约deal
    Route::post('/lever/dealall', 'Api\LeverController@dealAll'); //合约全部
    Route::post('/lever/submit', 'Api\LeverController@submit'); //合约下单
    Route::post('/lever/close', 'Api\LeverController@close'); //合约平仓
    ...
    Route::post('legal_send', 'Api\LegalDealController@postSend')->middleware(['demo_limit']); //商家发布法币交易信息
    Route::get('legal_deal_info', ['uses' => 'Api\LegalDealController@legalDealSendInfo', 'middleware' => ['check_user', 'demo_limit']]); //法币交易信息详情
    Route::post('do_legal_deal', ['uses' => 'Api\LegalDealController@doDeal', 'middleware' => ['check_user', 'demo_limit']]); //法币交易信息详情
    ...
    Route::post('user_legal_pay', ['uses' => 'Api\LegalDealController@userLegalDealPay', 'middleware' => ['check_user']])->middleware(['demo_limit']); //法币交易用户确认付款
    Route::post('user_legal_pay_cancel', ['uses' => 'Api\LegalDealController@userLegalDealCancel', 'middleware' => ['check_user']])->middleware(['demo_limit']); //法币交易用户取消订单
    ...
    Route::post('legal_deal_sure', ['uses' => 'Api\LegalDealController@doSure', 'middleware' => ['check_user']])->middleware(['demo_limit']); //商家确认收款
    Route::post('legal_deal_user_sure', ['uses' => 'Api\LegalDealController@userDoSure', 'middleware' => ['check_user']])->middleware(['demo_limit']); //用户确认收款
    Route::post('back_send', ['uses' => 'Api\LegalDealController@backSend', 'middleware' => ['check_user']])->middleware(['demo_limit']); //商家撤回发布
    Route::post('error_send', ['uses' => 'Api\LegalDealController@errorSend', 'middleware' => ['check_user']])->middleware(['demo_limit']); //商家撤回异常发布
...
//  Route::post('/transaction/deal', 'Api\TransactionController@deal');//deal
    ...
    //c2c交易
    Route::post('c2c_send', ['uses' => 'Api\C2cDealController@postSend', 'middleware' => ['check_user']])->middleware(['demo_limit']); //用户发布交易信息
    ...
    Route::post('c2c/do_legal_deal', ['uses' => 'Api\C2cDealController@doDeal', 'middleware' => ['check_user', 'demo_limit']]); //法币交易信息详情
    ...
    Route::post('c2c/user_legal_pay_cancel', ['uses' => 'Api\C2cDealController@userLegalDealCancel', 'middleware' => ['check_user', 'demo_limit']]); //法币交易用户取消订单
    Route::post('c2c/user_legal_pay', ['uses' => 'Api\C2cDealController@userLegalDealPay', 'middleware' => ['check_user', 'demo_limit']]); //法币交易用户确认付款
    Route::post('c2c/legal_deal_sure', ['uses' => 'Api\C2cDealController@doSure', 'middleware' => ['check_user', 'demo_limit']]); //商家确认收款
    Route::post('c2c/legal_deal_user_sure', ['uses' => 'Api\C2cDealController@userDoSure', 'middleware' => ['check_user', 'demo_limit']]); //用户确认收款
    Route::post('c2c/back_send', ['uses' => 'Api\C2cDealController@backSend', 'middleware' => ['check_user', 'demo_limit']]); //商家撤回发布
...
Route::group(['prefix' => 'admin', 'middleware' => ['admin_auth']], function () {
    ...
    Route::get('cashb', 'Admin\CashbController@index');
    ...
    //币种
    Route::get('currency1', 'Admin\CurrencyController@index');//首页
    ...
    Route::post('currency/match_add/{legal_id}', 'Admin\CurrencyController@postAddMatch')->middleware(['demo_limit']); //添加交易对
