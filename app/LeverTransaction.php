...
class LeverTransaction extends Model
{
    ...
    {
        return $this->belongsTo('App\CurrencyQuotation', 'currency', 'currency_id')->withDefault();
    }
    ...
    public function getNowPriceAttribute()
    {
        ...
        return db::table('currency_quotation')->where(['currency_id'=>$currency_id])->value('now_price');
    }
    ...
    public static function newPrice($legal_id, $currency_id, $price, $timestamp = null)
    {
        ...
        if (empty($legal_id) || empty($currency_id) || empty($price) || bc_comp($price, 0) <= 0) {
            return false;
        }
    ...
    public static function getUserProfit($user_id, $legal_id = 0, $currency_id = 0)
    {
        ...
        try {
            ...
            $user_profit = LeverTransaction::where('status', self::TRANSACTION)
                ...
                ->first();
            if (!$user_profit) {
                $levers = LeverTransaction::where("status", self::TRANSACTION)
                    ->where("user_id", $user_id)
                    ->where('update_price', '>', 0)
                    ->where(function ($query) use ($legal_id, $currency_id) {
                        $legal_id > 0 && $query->where("legal", $legal_id);
    ...
    {
        DB::beginTransaction();
        $trades = LeverTransaction::where("status", self::TRANSACTION)
            ->where("user_id", $user_id)
            ->where('update_price', '>', 0)
            ->where(function ($query) use ($legal_id, $currency_id) {
                $legal_id > 0 && $query->where("legal", $legal_id);
    ...
    public static function leverClose($lever_transaction)
    {
        try {
            ...
            if (bc_comp($last_price, 0) <= 0) {
                throw nex \Exception('当前价格获取异常,请稍后再试');
            }
