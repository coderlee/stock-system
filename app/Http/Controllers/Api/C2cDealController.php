    ...
    public function postSend(Request $request)
    {
        ...
        try {
            ...
            if ($type == 'sell') {   //如果发布出售信息
                ...
                if($wallet->lever_balance < $total_number){
                ...
                $data_wallet1 = [
                    ...
                    'lock_type' => 0,
                    'create_time' => time(),
                    'before' => $wallet->lever_balance,
                    'change' => -$total_number,
                    'after' => bc_sub($wallet->lever_balance, $total_number, 5),
                ];
                $data_wallet2 = [
                    'balance_type' =>  2,
                    'wallet_id' => $wallet->id,
                    'lock_type' => 1,
                    'create_time' => time(),
                    'before' => $wallet->lock_lever_balance,
                    'change' => $total_number,
                    'after' => bc_add($wallet->lock_lever_balance, $total_number, 5),
                ];

                $wallet->lever_balance = bc_sub($wallet->lever_balance,$total_number,5);
                
                $wallet->lock_lever_balance = bc_add($wallet->lock_lever_balance,$total_number,5);
                $wallet->save();
    ...
    public function doDeal(Request $request)
    {
        ...
        try {
            ...
            if ($legal_deal_send->type == 'buy') { //求购
                ...
                // do something
                if ($users_wallet->lever_balance < $number) {
                    DB::rollback();
                    return $this->error('您的余额不足');
                }
                if ($users_wallet->lock_lever_balance < 0) {
                    DB::rollback();
...
//                $legal_deal_send->surplus_number -= $number;
                ...
                $data_wallet1 = [
                    ...
                    'lock_type' => 0,
                    'create_time' => time(),
                    'before' => $users_wallet->lever_balance,
                    'change' => -$number,
                    'after' => bc_sub($users_wallet->lever_balance, $number, 5),
                ];
                $data_wallet2 = [
                    'balance_type' =>  2,
                    'wallet_id' => $users_wallet->id,
                    'lock_type' => 1,
                    'create_time' => time(),
                    'before' => $users_wallet->lock_lever_balance,
                    'change' => $number,
                    'after' => bc_add($users_wallet->lock_lever_balance, $number, 5),
                ];
//                $users_wallet->legal_balance -= $number;
                $users_wallet->lever_balance = bc_sub($users_wallet->lever_balance,$number,5);
//                $users_wallet->lock_legal_balance += $number;
                $users_wallet->lock_lever_balance = bc_add($users_wallet->lock_lever_balance,$number,5);
                $users_wallet->save();
                $legal_deal_send->save();
                AccountLog::insertLog(
                    [
    ...
    public function doSure(Request $request)
    {
        ...
        try {
            ...
            $data_wallet1 = [
                ...
                'lock_type' => 1,
                'create_time' => time(),
                'before' => $from_wallet->lock_lever_balance,
                'change' => -$legal_deal->number,
                'after' => bc_sub($from_wallet->lock_lever_balance,$legal_deal->number,5),
            ];
            $data_wallet2 = [
                'balance_type' =>2 ,
                ...
                'change' => $legal_deal->number,
                'after' => bc_add($user_wallet->lever_balance, $legal_deal->number, 5),
            ];
            //更新交易状态
            ...
            $from_wallet->lock_lever_balance=bc_sub($from_wallet->lock_lever_balance,$legal_deal->number,5);
            //增加用户法币余额
            $user_wallet->lever_balance = bc_add($user_wallet->lever_balance,$legal_deal->number,5);
            //日志
            AccountLog::insertLog(
                [
    ...
    public function userDoSure(Request $request)
    {
        ...
        try {
            ...
            $data_wallet1 = [
                ...
                'lock_type' => 1,
                'create_time' => time(),
                'before' => $user_wallet->lock_lever_balance,
                'change' => -$legal_deal->number,
                'after' => bc_sub($user_wallet->lock_lever_balance, $legal_deal->number, 5),
            ];
            $data_wallet2 = [
                'balance_type' =>2,
                ...
                'lock_type' => 0,
                'create_time' => time(),
                'before' => $seller_wallet->lever_balance,
                'change' => $legal_deal->number,
                'after' => bc_add($seller_wallet->lever_balance,$legal_deal->number,5),
            ];
            //更新交易状态
            $legal_deal->is_sure = 1;
            $legal_deal->update_time = time();
...
//            $user_wallet->lock_legal_balance -= $legal_deal->number;
            $user_wallet->lock_lever_balance = bc_sub($user_wallet->lock_lever_balance,$legal_deal->number,5);

            //增加商家法币余额
//            $seller->seller_balance += $legal_deal->number;
              $seller_wallet->lever_balance = bc_add($seller_wallet->lever_balance,$legal_deal->number,5);
            //日志
            AccountLog::insertLog(
                [
                    'user_id' => $user->id,
    ...
    public function backSend(Request $request)
    {
        ...
        try {
            ...
            if ($legal_send->type == 'sell') {
                ...
                if($wallet->lock_lever_balance < $legal_send->total_number){
                    DB::rollback();
                ...
                $data_wallet1 = [
                    ...
                    'after' => bc_add($wallet->lever_balance, $legal_send->total_number, 5),
                ];
                $data_wallet2 = [
                    'balance_type' =>  2,
                    'wallet_id' => $wallet->id,
                    ...
                    'before' => $wallet->lock_lever_balance,
                    'change' => -$legal_send->total_number,
                    'after' => bc_sub($wallet->lock_lever_balance, $legal_send->total_number, 5),
                ];

                $wallet->lever_balance = bc_add($wallet->lever_balance, $legal_send->total_number, 5);
                
                $wallet->lock_lever_balance = bc_sub($wallet->lock_lever_balance, $legal_send->total_number, 5);
                $wallet->save();
                // AccountLog::insertLog(['user_id' => $user_id, 'value' => $total_number * -1, 'info' => '用户发布c2c交易法币出售，扣除法币余额', 'type' => AccountLog::C2C_DEAL_SEND_SELL, 'currency' => $currency_id]);
                AccountLog::insertLog(
                    [
