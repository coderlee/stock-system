...
class LeverController extends Controller
{
    ...
    public function deal()
    {
        ...
        $my_transaction = LeverTransaction::with('currency1')
            ->with('user')
            ->orderBy('id', 'desc')
...
//        dd($lever_transaction);
    ...
    public function myTrade()
    {
        ...
        $lever_transaction = LeverTransaction::with('currency1')
    ...
    public function submit()
    {
        ...
        $legal_id = Input::get("legal_id");
        $currency_id = Input::get("currency_id");
		$bond = Input::get("bond");//mobile保证金
		$fee = Input::get("fee");//mobile手续费
...
//        $trade_fee = bc_mul($all_money, $lever_trade_fee_rate);
        $trade_fee = bc_mul($share, $currency_match->level_shouxu);
        DB::beginTransaction();
        try {
...
//            $caution_money = bc_div($all_money, $multiple); //保证金
            $caution_money = bc_mul($share, $currency_match->level_baozheng); //保证金
            $shoud_deduct = bc_add($caution_money, $trade_fee); //保证金+手续费
			$bondall=$bond+$fee;
			if(($user_lever-$bondall)<0){
				throw new \Exception($currency_match->legal_name . "余额不足,不能小于" . $bondall . '(手续费:' . $fee . ')');
			}			
...
//            if ($target_profit_price <= $lever_transaction->price || $target_profit_price <= $lever_transaction->update_price) {
            if ($target_profit_price <= $lever_transaction->origin_price) {
                return $this->error('买入(做多)止盈价不能低于开仓价');
            }
//            if ($stop_loss_price >= $lever_transaction->price || $stop_loss_price >= $lever_transaction->update_price) {
            if ($stop_loss_price >= $lever_transaction->origin_price) {
                return $this->error('买入(做多)止亏价不能高于开仓价');
            }
        } else {
            //卖出
//            if ($target_profit_price >= $lever_transaction->price || $target_profit_price >= $lever_transaction->update_price) {
            if ($target_profit_price >= $lever_transaction->origin_price) {
                return $this->error('卖出(做空)止盈价不能高于开仓价');
            }
//            if ($stop_loss_price <= $lever_transaction->price || $stop_loss_price <= $lever_transaction->update_price) {
            if ($stop_loss_price <= $lever_transaction->origin_price) {
                return $this->error('卖出(做空)止亏价不能低于开仓价');
            }
        }
        ...
        try {
            ...
            DB::transaction(function () use ($user_id, $id) {
                ...
                $result = change_wallet_balance(
                    ...
                    '合约' . $lever_trade->type_name . '委托撤单,退回手续费',
                    false,
                    0,
                    0,
                    '',
                ...
                $result = change_wallet_balance(
                    ...
                    AccountLog::LEVER_TRANSACTIO_CANCEL,
                    '合约' . $lever_trade->type_name . '委托撤单,退回保证金',
                    false,
                    0,
                    0,
                    '',
