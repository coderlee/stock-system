...
class RobotController extends Controller
{
    ...
    public function start()
    {
        ...
        DB::beginTransaction();
        try {
            if ($robot->status == Robot::START) {
                $robot->status = Robot::STOP;
                $info          = $robot->save();
            } else {
                $robot->status = Robot::START;
                $info          = $robot->save();

                $log_path = base_path('robot/' . date('Ymd') . '.log');

                $process = new Process("nohup php artisan robot {$robot->id} >{$log_path} 2>&1 &", base_path());

//                $process  = new Process("nohup php artisan robot {$robot->id} >./robot.log 2>&1 &", base_path());
                $process->start();
            }
