...
class NewsController extends Controller
{
    ...
    public static function newsList($cId = 0, $keyword = '', $num = 0)
    {
        $keyword = '%' . $keyword . '%';
        if($num != 0) {
            if($cId == 0) {
                $news = NewsModel::where('title', 'like', $keyword)->orderBy('id', 'desc')->paginate($num);
            } else {
                $news = NewsModel::where('c_id', $cId)->where('title', 'like', $keyword)->orderBy('id', 'desc')->paginate($num);
            }
        } else {
            if($cId == 0) {
                $news = NewsModel::where('title', 'like', $keyword)->orderBy('id', 'desc')->get();
            } else {
                $news = NewsModel::where('c_id', $cId)->where('title', 'like', $keyword)->orderBy('id', 'desc')->get();
            }
        }
        return $news;
    }
    ...
    */
    ...
    public function index(Request $request, $c_id = 0, $keyword = '')
    {
        ...
        $news = self::newsList($c_id, $keyword, 10);
        $count = count($news);
        $data = [
            'count'=> $count,
            'news' => $news,
    ...
    */
    ...
    {
        $cateList = NewsCategory::all();
        return view('admin.news.add', ['cateList' => $cateList]);
    }
